...
# score.pl for the cross lingual lexical substitution task SemEval 2010 
# by Diana McCarthy 9 Sept 2009
# Adapted from code written by Diana McCarthy 
# for English Lexical Substitution Task SemEval 2007
use strict;
...
if ($scoretype) {
 ...
 #   scorebest($goldfile,$ansfile,$subana,$E); 
    ...
    print "Scoring 'oot' goldfile = $goldfile sysfile = $ansfile\n"; 
...
sub scoreOOT {
  ...
  if ($dupflag) {
      print "NB OOT file contains duplicates on $dupflag lines\n";
  }
  $precision = $corr / $itemsattempted;
  $precision = myround($precision,$dp);
  $recall = $corr / $totitems;
